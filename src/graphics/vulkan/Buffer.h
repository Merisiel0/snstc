...
public:
  ...
  static void init(std::weak_ptr<Device> device, std::weak_ptr<Allocator> allocator,
  ...
  Buffer(std::vector<T> data, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage) :
    ...
    Buffer* staging =
      new Buffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
