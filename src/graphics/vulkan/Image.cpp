...
void Image::init(std::shared_ptr<Device> device,
                 std::weak_ptr<Allocator> allocator,
  ...
  const std::vector<VkFormat> preferredColorFormats = {
      ...
      VK_FORMAT_R8G8B8A8_SNORM,   VK_FORMAT_R8G8B8A8_USCALED,
  ...
  for (const auto& format : preferredColorFormats) {
    ...
    VkResult result = vkGetPhysicalDeviceImageFormatProperties(
        device->physicalDevice()->handle, format, VK_IMAGE_TYPE_2D,
...
Image::Image(const Swapchain& swapchain,
             ImageType type,
  ...
  VK_CHECK(vmaCreateImage(allocatorSptr->handle, &createInfo, &allocCreateInfo,
                          &handle, &_allocation, &_info));

  VkImageViewCreateInfo viewCreateInfo = getViewCreateInfo();
  VK_CHECK(
      vkCreateImageView(deviceSptr->handle, &viewCreateInfo, nullptr, &view));
}

Image::Image(const char* path) {
  std::shared_ptr<Device> deviceSptr = getShared(_device);
  std::shared_ptr<Allocator> allocatorSptr = getShared(_allocator);
  std::shared_ptr<ImmediateSubmit> immediateSubmitSptr =
      getShared(_immediateSubmit);

  int width, height;
  stbi_uc* pixels =
      stbi_load(path, &width, &height, &_channelAmount, STBI_rgb_alpha);
  _channelAmount = 4;
  ...
  size_t imageSize =
      static_cast<size_t>(_extent.width * _extent.height * _channelAmount);
  ...
  VK_CHECK(vmaCreateImage(allocatorSptr->handle, &createInfo,
                          &allocationCreateInfo, &handle, &_allocation,
                          &_info));

  immediateSubmitSptr->submit([&stagingBuffer,
...
void Image::transitionLayout(CommandBuffer* commandBuffer,
...
void Image::transitionFormat(CommandBuffer* commandBuffer, VkFormat newFormat) {
  ...
  immediateSubmitSptr->submit(
      [&layout, &stagingImg, this](CommandBuffer* commandBuffer) {
...
VkRenderingAttachmentInfo Image::getRenderingAttachmentInfo(
  ...
  attachmentInfo.loadOp =
      doClear ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_LOAD;
...
VkRenderingInfo Image::getRenderingInfo(const Image& color,
                                        const Image& depth,
                                        const VkClearValue& clear,
                                        bool doClear) {
  ...
  renderInfo.pColorAttachments = new VkRenderingAttachmentInfo(
      color.getRenderingAttachmentInfo(clear, true));
  renderInfo.pDepthAttachment = new VkRenderingAttachmentInfo(
      depth.getRenderingAttachmentInfo(VkClearValue{1.0f, 0}, true));
...
void Image::blitTo(CommandBuffer* commandBuffer,
...
void Image::copyTo(CommandBuffer* commandBuffer, Image* image) const {
  ...
  vkCmdCopyImage(commandBuffer->handle, handle, _layout, image->handle,
                 image->_layout, 1, &region);
}
...
void Image::copyToBuffer(CommandBuffer* commandBuffer, Buffer* buffer) const {
  ...
  vkCmdCopyImageToBuffer(commandBuffer->handle, handle, _layout, buffer->handle,
                         1, &region);
}
...
void Image::save(const char* path) {
  ...
  if (_format != VK_FORMAT_R8G8B8A8_SRGB)
    imageSize *= 2;
  imageSize += 1;
  Buffer* staging = new Buffer(imageSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT,
...
void Image::clear(CommandBuffer* commandBuffer, Color color) {
  ...
  vkCmdClearColorImage(commandBuffer->handle, handle, _layout, &clearColor, 1,
                       &clearRange);
}
